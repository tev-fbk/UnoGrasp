...
def make_supervised_data_module(
    ...
    # 第一个作为训练集
    train_args = copy.deepcopy(data_args)
    train_args.dataset_use = dataset_names[0]
    train_dataset = LazySupervisedDataset(tokenizer=tokenizer, data_args=train_args)

    # 第二个作为验证集（可选）
    eval_dataset = None
    if len(dataset_names) > 1:
        eval_args = copy.deepcopy(data_args)
        eval_args.dataset_use = dataset_names[1]
